...
def combineClustering(cluster1, cluster2):
    cluster["SUMSQ"][0] = float(cluster1["SUMSQ"][0]) + float(cluster2["SUMSQ"][0])
    cluster["SUMSQ"][1] = float(cluster1["SUMSQ"][1]) + float(cluster2["SUMSQ"][1])
    cluster["SUMSQ"][2] = float(cluster1["SUMSQ"][2]) + float(cluster2["SUMSQ"][2]) 
    cluster["SUM"][0] = cluster1["SUM"][0] + cluster2["SUM"][0]
    cluster["SUM"][1] = cluster1["SUM"][1] + cluster2["SUM"][1]
    cluster["SUM"][2] = cluster1["SUM"][2] + cluster2["SUM"][2]
    cluster["N"] = cluster1["N"] + cluster2["N"] 
    cluster["points"].append(cluster1["points"])
    cluster["points"].append(cluster2["points"])
    return cluster
